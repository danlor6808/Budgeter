...
namespace Budgeter.Controllers
{
    ...
    public class AccountController : MyBaseController
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, string returnUrl, string code)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //Attaches all invitations to user
                    if (db.Invitations.Any(u => u.Email == model.Email))
                    {
                        var inv = db.Invitations.Where(u => u.Email == model.Email);
                        foreach (var invitation in inv)
                        {
